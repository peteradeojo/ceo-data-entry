...
def soup4_find(address):
  html = requests.get(f"https://www.google.com/search?q=who is the ceo of {remove_dot_com(address[1])}")
  soup = BeautifulSoup(html.text, features="html.parser")

  el = soup.css.select_one("#main > div:nth-of-type(3) span")
  if el: #and test_valid_name(el.text):
    # results.append(f"{el.text}\n{address}\n")
    return inspect_result(el.text)
  else:
    return input(f"Missing {address}: ")
